...
```{r exercise 07-4}
p <- median(msleep$bodywt)
msleep[ msleep$bodywt> p, ]
...
```{r exercise 10-3}
mean_thing <-  mean(msleep$sleep_total)
data_with_zscoresleep <- msleep %>% 
  mutate(averagesleep = mean(sleep_total), stdvsleep = sd(sleep_total), zscoresleep = (sleep_total-mean(sleep_total))/sd(sleep_total))
view(data_with_zscoresleep)
...
data_w_bodywtzscore <- msleep %>% 
mutate( zscorebodywt = (bodywt - mean(bodywt))/sd(bodywt))
view(data_w_bodywtzscore)
```
